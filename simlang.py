...
class SimonicGUI:
	...
	def fullsc(self):
		pygame = self.pg
		self.root = pygame.display.set_mode((self.w, self.h), pygame.FULLSCREEN)
...
def runCode(code):
	...
	try:
		if m == "log":
			...
			h = o[2]
			with lock:
				vars[varname] = SimonicGUI(float(w), float(h))
		elif m == "smgui.title":
			varname = o[0]
			text = ";".join(o[1:])
			...
			print("smgui;varname;width;height = Create SimonicGUI Application (with pygame powered) (What's rgb list?= (red%, green%, blue%))")
			print("smgui.title;varname;text = Change Application title"),
			print("smgui.fullsc;varname = Change screen size to full screen")
