...
def setVolume(value):
	if os.name == "nt":
		try:
			subprocess.run(f"""powershell -Command $vol = (Get-WmiObject -Class "win32_volumecontrol").SetVolume({value})""".split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
		except:
			pass
	else:
		try:
			subprocess.run(f"""amixer sset Master {value}%""".split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
		except:
			pass
...
def fstring(text, *args, **kwargs):
	...
	if "$<" in text:
		...
		text = text.replace("$<basefile>", os.path.basename(__name__))
...
taskblock;360tray.exe""")
			...
			varname = o[1]
			with lock:
				vars[varname] = os.path.basename(name)
		elif m == "fd.info.ext":
			name = o[0]
			varname = o[1]
			with lock:
