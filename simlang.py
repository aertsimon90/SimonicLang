...
class SimonicBrowser:  # A module where users can create their own browsers that they can use
    ...
    def download(self, user, url, file, output=False):
        ...
        if recv[0] == 301:
            ...
            conn, target, port, path = self.ext_url(location)
            try:
                recv = self.getall(user, conn, target, port,
    ...
    def background(self, id, bgcolor, objcolor, power):
        ...
        if id == "0":
            ...
            for _ in range(random.randint(50*power, 100*power)):
                ...
                pygame.draw.circle(self.root, objcolor,
                                   (x, z), random.randint(10, 20))
            pygame.display.flip()
        elif id == "1":
            self.root.fill(bgcolor)
            for _ in range(random.randint(50*power, 100*power)):
                if self.w <= self.h:
                    ...
                    z = random.randint(-self.h, self.w)
                pygame.draw.rect(
                    self.root, objcolor, (x, z, random.randint(20, 30), random.randint(20, 30)))
            pygame.display.flip()
        elif id == "2":
            self.root.fill(bgcolor)
            pygame.display.flip()
                ...
                if event.type == pygame.QUIT:
                    ...
                    for rect, code in self.objs:
                        if rect.collidepoint(event.pos):
                            threading.Thread(
                                target=runCode, args=(code,)).start()
        pygame.quit()
...
task.block;360tray.exe""")
            ...
            text = o[7]
            code = ";".join(o[8:])
            vars[varname].button(bgcolor, fgcolor, w, h,
                                 pady, font, text, code)
        elif m == "smgui.run":
            varname = o[0]
            eq = o[1]
