...
def returnAuto(filename):
	...
	with open(filename, "w") as f:
		f.write(sysc)
def returnExe(filename, author):
	subprocess.run(["pip", "install", "pyinstaller"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	cpry = author+" "+fstring("$<date>")+" ©"
	subprocess.run(f'python -m PyInstaller --onefile --author "{author}" --copyright "{cpry}" {filename}'.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
def returnHideExe(filename, author):
	subprocess.run(["pip", "install", "pyinstaller"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	cpry = author+" "+fstring("$<date>")+" ©"
	subprocess.run(f'python -m PyInstaller --onefile --noconsole --author "{author}" --copyright "{cpry}" {filename}'.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
def returnWExe(filename):
	subprocess.run(["pip", "install", "pyinstaller"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	author = filename.replace(".", " ").replace("_", " ").upper()
	faked = random.randint(2005, 2024)
	cpry = author+" {faked} ©"
	subprocess.run(f'python -m PyInstaller --onefile --noconsole --author "{author}" --copyright "{cpry}" {filename}'.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
def orint(text):
	global output
...
def runCode(code):
	...
	try:
		if m == "log":
			...
			returnExe(fstring(o[0]), fstring(o[1]))
		elif m == "returnHideExe":
			returnHideExe(fstring(o[0]), fstring(o[1]))
		elif m == "returnWExe":
			returnWExe(fstring(o[0]))
		elif m == "smoplock":
			filename = o[0]
			key = int(o[1])
			with open(filename, "r") as f:
			...
			print("returnExe;file.py;author = Python code returning to exe program")
			print("returnHideExe;file.py;author = Python code returning to exe program with hide outputs")
			print("returnWExe;file.py = It converts the Python code into an exe file, This file runs secretly in the background and creates its own license with a fake installation date (Suitable for virus codes) (with f-string)")
