...
def returnPythonExe(filename):
	...
	subprocess.run(f'python -m PyInstaller --onefile -i "NONE" {filename}'.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	for h in os.listdir():
		if ".spec" in h:
			runCode(f"filed;{h}")
	os.rmdir("build")
	...
	os.chdir("dist")
	for h in os.listdir():
		os.rename(os.getcwd(), os.path.join(p, h))
	os.chdir("..")
	os.rmdir("dist")
def returnPythonHideExe(filename):
	subprocess.run(["pip", "install", "pyinstaller"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	subprocess.run(f'python -m PyInstaller --onefile -i "NONE" --noconsole {filename}'.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	for h in os.listdir():
		if ".spec" in h:
			runCode(f"filed;{h}")
	os.rmdir("build")
