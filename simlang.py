...
def returnPythonExe(filename):
	loadAllMods()
	subprocess.run([f"python", "-m", "PyInstaller", "--onefile", filename])
	remexea()
def returnPythonHideExe(filename):
	loadAllMods()
	subprocess.run([f"python", "-m", "PyInstaller", "--onefile", "--noconsole", filename])
	remexea()
def returnSlExe(filename, ab=0):
	with open(filename, "rf") as f:
		a = f.read()
	returnAuto(filename, ab)
	returnPythonExe(ilename)
	with open(filename, "w") as f:
		f.write(a)
