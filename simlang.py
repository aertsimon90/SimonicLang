...
import sys, os, subprocess, platform, socket, json, threading, time, random, webbrowser, secrets, shutil, sqlite3
...
def loadMod(id, name):
	...
	with lock:
		mods.append(id)
def loadAllMods():
	mods = {"psutil": 342, "fake_useragent": 83, "datetime": 2, "requests": 0, "keyboard": 245, "tkinter": 3422, "flask": 963}
	ts = []
...
def runCode(code):
	...
	try:
		if m == "log":
			...
			with lock:
				vars[varname] = recv
		elif m == "SimonServer.SetOpenFile":
			ip = o[0]
			pawd = o[1]
			file = o[2]
			c = ";".join(o[3:]).replace("\n", "/n/*")
			...
			recv = "\r\n\r\n".join(recv.split("\r\n\r\n")[1:])
			print(recv)
		elif m == "SimonServer.GetOpenFile":
			ip = o[0]
			file = o[1]
			varname = o[2]
			s = SimonServer()
			...
			print("SimonServer.GetOpenFile;ip or dns;filename;varname = Get content of public file from server")
			print("SimonServer.Open;ip or dns;varname = Get content of public main named file from server")
			print("SimonServer.GetAll;ip or dns;password;varname = Get all data from server.")
