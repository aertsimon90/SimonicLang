...
import sys, os, subprocess, platform, socket, json, threading, time, random, webbrowser, secrets
...
def pageRun(code):
	...
	if 527 in mods:
		...
		try:
			subprocess.run(["pip", "install", "tkinter"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
			with lock:
				mods.append(527)
		except:
			pass
	exec("""import """+"""tkinter as tk
from """+"""tkinter import scrolledtext

class page:
	def __init__(self, name="UnknowPage"):
		self.root = tk.Tk()
	...
	def label(self, id, text):
		labell = tk.Label(self.root, text=text)
		labell.pack()
		self.labels[id] = labell
	def labelset(self, id, text):
		labell = self.labels[id]
		labell.config(text=text)
		self.labels[id] = labell
	def button(self, id, command, text, width=5):
		buttonn = tk.Button(self.root, text=text, command=command, width=width)
		buttonn.pack()
		self.buttons[id] = buttonn
	def buttonset(self, id, command, text, width=5):
		buttonn = self.buttons[id]
		buttonn.config(text=text, command=command)
		self.buttons[id] = buttonn
	def labelsc(self, id, disable=True, width=30, height=5):
		if disable:
			labell = scrolledtext.ScrolledText(self.root, wrap=tk.WORD, width=width, height=height, state=tk.DISABLED)
		else:
			labell = scrolledtext.ScrolledText(self.root, wrap=tk.WORD, width=width, height=height)
		labell.pack()
		self.labelscs[id] = labell
	def labelscset(self, id, text):
		labell = self.labelscs[id]
		if labell.cget("state") == tk.DISABLED:
			disable=True
		else:
			disable=False
		labell.config(state=tk.NORMAL)
		labell.delete(1.0, tk.END)
		labell.insert(tk.END, text)
		if disable:
			labell.config(state=tk.DISABLED)
	def labelscget(self, id):
		labell = self.labelscs[id]
		return labell.get("1.0", tk.END)
	def input(self, id, width=30):
		inputt = tk.Entry(self.root, width=width)
		inputt.pack()
		self.inputs[id] = inputt
	def inputreset(self, id):
		inputt = self.inputs[id]
		inputt.delete(0, tk.END)
		self.inputs[id] = inputt
	def inputset(self, id, text):
		inputt = self.inputs[id]
		inputt.delete(0, tk.END)
		inputt.insert(0, text)
		self.inputs[id] = inputt
	def inputget(self, id):
		inputt = self.inputs[id]
		return inputt.get()
	def run(self):
		self.root.mainloop()
...
class SimonCloud:
	...
	def com(self, c): # Bash Command Runner
		...
		self.s.sendall(p)
	def recv(self):
		recv = self.s.recv(999999999)
		return recv.decode().split("\r\n\r\n")[1].replace("/n/*", "\n")
...
def fstring(text):
	...
	if "$<all>" in text:
		txt = []
		for h in ["basefile", "sys.name", "sys.sys", "sys.ver", "sys.node", "sys.user", "sys.path", "sys.prcsr", "sys.pyver", "sys.slver", "sys.mac", "sys.arch", "sys.ip", "sys.host", "ram.b", "ram.kb", "ram.mb", "ram.gb", "ramav.b", "ramav.kb", "ramav.mb", "ramav.gb", "ramus.b", "ramus.kb", "ramus.mb", "ramus.gb", "prcsc"]:
			try:
...
def returnHideExe(filename):
	subprocess.run(["pip", "install", "pyinstaller"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	subprocess.run(["python", "-m", "PyInstaller", "--onefile", "--noconsole", filename], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
def orint(text):
	global output
	text = text.replace("$<n>", "\n")
...
def runCode(code):
	...
	try:
		if m == "log":
			...
			returnExe(fstring(ot))
		elif m == "returnHideExe":
			returnHideExe(fstring(ot))
		elif m == "smoplock":
			filename = o[0]
			key = int(o[1])
			with open(filename, "r") as f:
			...
			pageRun(ot.replace("/n/", "\n"))
		elif m == "pageCodingA":
			with open(ot, "r") as f:
				pageRun(f.read().replace("/n/", "\n"))
		elif m == "clearIn":
			with lock:
				output.flush()
				vars = {}
			...
			with lock:
				...
				try:
					...
					print("Loading - Bad")
				print("File returning test:")
				try:
					with open("test", "w") as f:
						f.write("log;Hello, World!")
				except:
					pass
				try:
					returnAuto("test")
					print("returnAuto - Good")
				except:
					print("returnAuto - Bad")
				try:
					returnExe("test")
					print("returnExe - Good")
				except:
					print("returnExe - Bad")
				print("Removing Testing File...")
				try:
					with open("test", "w") as f:
						f.write("")
				except:
			...
			print("returnAuto;file.sl = Returning to auto running python code (SimonicLang to Python)")
			print("returnExe;file.py = Python code returning to exe program")
			print("returnHideExe;file.py = Python code returning to exe program with hide outputs")
...
optilogf;Finded Random Server: $<random.server>$<n>""")
			print("""
When coding a page in SimonicLang, there are two methods:
1. pageCodingM;code... = Manual, meant for coding within the main code, and /n/ can be used as a newline character since each code is not automatically on a new line.
2. pageCodingA;filename... = Auto, intended for using the content of a different file within the main code. Its functionality is the same, but /n/ is not needed.

When coding the page, you should use Python code. Additionally, within the Python code, you can start SimonicLang code directly in the main code using the runCode(codeText) function.

Remember to automatically import the tkinter module while coding the page, and for simplifying the usage of this module, we have created a class, which you can find below:
...
class page:
	...
	def run(self):
		self.root.mainloop()
""")
			print("The above is a class code that you can use to code the page within SimonicLang, and this code will automatically be added to your class code environment, you do not need to add it.")
	except Exception as e:
		if errors:
			orint(str(e))
