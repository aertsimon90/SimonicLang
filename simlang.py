...
def returnPythonExe(filename):
	loadAllMods()
	cc = "python -m PyInstaller --onefile --hidden-import=tkinter "+filename
	subprocess.run(cc.split())
	time.sleep(3)
	remexea()
def returnPythonHideExe(filename):
	loadAllMods()
	cc = "python -m PyInstaller --onefile --hidden-import=tkinter --noconsole "+filename
...
def runCode(code):
	...
	try:
		if m == "log":
			...
			print("fullBoxUnClose;backgroundColor;textColor;text = Display text to all of screen (Un closed)")
