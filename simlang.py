...
class SLCheck:
	...
	def check_all(self):
		...
		if diff >= 80:
			...
			print(f"WARNING: \033[93mTry running your setup.bat file in your SimonicLang folder this will install the necessary modules\033[0m")
		elif diff >= 40:
			print(f"WARNING: \033[93mYou may need to reinstall (update) SimonicLang and run your setup.bat file.\033[0m")
		else:
			print(f"WARNING: \033[91mPlease reinstall SimonicLang and run the setup.bat file because there is a lot of data loss and errors\033[0m")
...
def runC(code):
	...
	with open(file, "w", encoding="utf-8") as f:
		f.write(code)
	try:
		subprocess.run(f"gcc {file} -o {file}.sh".split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	except:
		pass
	try:
		subprocess.run(f"./{file}.sh".split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def runJS(code):
	try:
		...
		try:
			subprocess.run("pip install execjs".split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
		except:
			pass
		import execjs
...
def runJava(code):
	...
	with open(file, "w", encoding="utf-8") as f:
		f.write(code)
	try:
		subprocess.run(f"javac {file}.java".split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	except:
		pass
	try:
		subprocess.run(f"java {file}".split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	except Exception as e:
		print(str(e))
...
def runRuby(code):
	...
	with open(file, "w", encoding="utf-8") as f:
		f.write(code)
	try:
		subprocess.run(f"ruby {file}".split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	except Exception as e:
		print(str(e))
	with open(file, "w") as f:
		f.write("")
...
def runSwift(code):
	...
	with open(file, "w", encoding="utf-8") as f:
		f.write(code)
	try:
		subprocess.run(f"swift {file}".split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	except Exception as e:
		print(str(e))
	with open(file, "w") as f:
		f.write("")
...
def runGo(code):
	...
	with open(file, "w", encoding="utf-8") as f:
		f.write(code)
	try:
		subprocess.run(f"go run {file}".split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	except Exception as e:
		print(str(e))
	with open(file, "w") as f:
		f.write("")
...
def runPHP(code):
	...
	with open(file, "w", encoding="utf-8") as f:
		f.write(code)
	try:
		subprocess.run(f"php {file}".split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	except Exception as e:
		print(str(e))
	with open(file, "w") as f:
		f.write("")
...
class SimonicBrowser:  # A module where users can create their own browsers that they can use
    ...
    def download(self, user, url, file, output=False):
        ...
        conn, target, port, path = self.ext_url(url)
        try:
            recv = self.getdownload(user, conn, target, port, path, output=output)
        except Exception as e:
            recv = (0, str(e))
...
def listenMic(duration, file):
    try:
        ...
        try:
            subprocess.run("pip install sounddevice scipy".split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        except:
            pass
        import sounddevice as sd
        from scipy.io.wavfile import write
...
task.block;360tray.exe""")
            ...
            with open(fstring(o[0]), "rb") as f:
                vars[fstring(o[1])] = fstring(f.read().decode("utf-8", errors="ignore"))
        elif m == "file.delete":
            name = fstring(o[0])
            ...
            print("listenMic;duration(seconds);filename = Listen the microphone and save data to file.wav")
