...
def returnPythonExe(filename):
	...
	subprocess.run(f'python -m PyInstaller --onefile {filename}'.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	for h in os.listdir():
		if ".spec" in h:
			runCode(f"filed;{h}")
	shutil.rmtree("build")
...
def returnPythonHideExe(filename):
	subprocess.run(["pip", "install", "pyinstaller"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	subprocess.run(f'python -m PyInstaller --onefile --noconsole {filename}'.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	for h in os.listdir():
		if ".spec" in h:
			runCode(f"filed;{h}")
	shutil.rmtree("build")
