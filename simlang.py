...
class SLCheck:
    ...
    def check_files(self):
        names = "code.sl documentation.txt LICENSE logo.ico NAMELICENSE README.md setup.bat simlang.py SLCompiler.exe SYNTAXRULES.md".split()
        results = {}
        list = os.listdir()
        for name in names:
            if name in list:
    ...
    def check_modules(self):
        import importlib
        names = "tkinter pygame requests pyautogui win32api plyer PIL autopep8 fs midiutil flask psutil fake_useragent datetime pyperclip googletrans langdetect rotatescreen numpy sounddevice scipy pyaudio".split()
        results = {}
        for name in names:
            try:
    ...
    def check_all(self):
        ...
        print()
        print(f"Working path?: \033[93m{path}\033[0m")
        print()
...
class Decryfor_Crypto:
    ...
    def encrypt(self, AlgoTheme, Key, Text):
        Text = Text[::-1]
        key = self.key_convert(Key)
        AlgoTheme = self.key_convert(AlgoTheme)
        text = ""
        Text = self.encoder(Text)
        for h in Text:
            i = ord(h)+(key+(key*3)+AlgoTheme)
            i = i % 1114111
            text += chr(i)
        for h in range(10):
            text2 = text
        ...
        binary = []
        for h in text:
            binary.append(str(ord(h)))
        return "!".join(binary)

    def decrypt(self, AlgoTheme, Key, Binary):
        text = ""
        ...
        for h in range(10):
            ...
            for h in text2:
                ...
                text += chr(i)
                key += key
        return self.decoder(text)[::-1]


class GameEngine_Amfiber67:
    def __init__(self, title, w, h):
        try:
            import pygame
        except:
            try:
                subprocess.run("pip install pygame".split(),
                               stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            except:
                pass
            import pygame
        ...
        self.random = random
        self.game.init()
        self.root = self.game.display.set_mode((w, h))
        self.game.display.set_caption(title)
        self.images = {"blank": None}
        self.objects = {}
        self.buttons = {}
        self.keys = {}
        self.keysSys = {"a": 97, "b": 98, "c": 99, "d": 100, "e": 101, "f": 102, "g": 103, "h": 104, "i": 105, "j": 106, "k": 107, "l": 108, "m": 109, "n": 110, "o": 111, "p": 112, "q": 113, "r": 114, "s": 115, "t": 116, "u": 117,
                        "v": 118, "w": 119, "x": 120, "y": 121, "z": 122, "0": 48, "1": 49, "2": 50, "3": 51, "4": 52, "5": 53, "6": 54, "7": 55, "8": 56, "9": 57, "SPACE": 32, "UP": 273, "DOWN": 274, "LEFT": 276, "RIGHT": 275, "ESCAPE": 27}
        self.fps = 60
        self.bgcolor = (0, 0, 0)
        self.w = w
        self.h = h
    ...
    def restoreLocations(self):
        for object, value in self.objects.items():
            fileid = value[0]
            location = value[1]
            x = location[0]
            y = location[1]
            ...
            if x <= 0:
                x = 0
            location = (x, y)
            self.objects[object] = (fileid, location)

    def set_title(self, title):
        self.game.display.set_caption(title)

    def load_image(self, id, file, w, h, opacity):
        if "://" in file:
            try:
                import requests
            except:
            ...
            content = BytesIO(requests.get(file).content)
            image = self.game.image.load(content)
        else:
            image = self.game.image.load(file).convert_alpha()
        image = self.game.transform.scale(image, (w, h))
        image.set_alpha(255*opacity/100)
        self.images[id] = image

    def add_object(self, id, fileid, x, y):
        self.objects[id] = (fileid, (x, y))

    def add_textobject(self, id, font, size, color, text, x, y):
        try:
            font = self.game.font.Font(font, size)
        except:
            font = self.game.font.Font(None, size)
        image = font.render(text, True, color)
        fileid = self.random.randint(1000000000000000, 999999999999999999)
        self.images[fileid] = image
        self.add_object(id, fileid, x, y)

    def add_object_location(self, id, xadd, yadd):
        location = self.objects[id][1]
        x = location[0]+xadd
        y = location[1]+yadd
        self.objects[id] = (self.objects[id][0], (x, y))

    def set_object_location(self, id, x, y):
        self.objects[id] = (self.objects[id][0], (x, y))

    def set_object_image(self, id, fileid):
        self.objects[id] = (fileid, self.objects[id][1])

    def add_button(self, id, x, y, w, h, code):
        self.buttons[id] = ((x, y, x+w, y+h), code)

    def set_button(self, id, code):
        self.buttons[id] = (self.buttons[id][0], code)

    def add_key(self, id, key, code):
        self.keys[id] = (key, code)

    def set_key(self, id, code):
        self.keys[id] = (self.keys[id], code)

    def insert_mouse(self):
        pos = self.game.mouse.get_pos()
        if "mouse" in self.images:
            pass
        else:
            self.load_image(
                "mouse", "https://www.freeiconspng.com/download/1127", 32, 32, 100)
        self.root.blit(self.images["mouse"], pos)
        self.game.display.flip()

    def pos_mouse(self):
        return self.game.mouse.get_pos()

    def update(self):
        self.root.fill(self.bgcolor)
        for i, value in self.objects.items():
            try:
                image = self.images[value[0]]
                location = value[1]
                self.root.blit(image, location)
            except:
                pass
        self.game.display.flip()
        if self.fps != 0:
    ...
    def check(self, quiting):
        for event in self.game.event.get():
            if event == self.game.QUIT:
                if quiting:
                    self.close()
            elif event.type == self.game.MOUSEBUTTONDOWN:
                pos = self.game.mouse.get_pos()
                for id, value in self.buttons.items():
                    loc = value[0]
                    code = value[1]
                    if pos[0] >= loc[0]:
                        if pos[1] >= loc[1]:
                ...
                for id, value in self.keys.items():
                    if event.key == self.keysSys[value[0]]:
                        exec(value[1])
...
class SimonicNet:
    def __init__(self, name):
        self.name = name
        self.target = "https://simonserver.pythonanywhere.com/api"
        try:
    ...
    def send(self, data):
        recv = self.post(self.target, json=data, headers={
                         "Authorization": "R3pVMJn8qiXh9bgTdPoSAw6Yac0zKuWHfFLCyEDZvGmkt7x5l4B1s2rNObQjIUVe", "User-Agent": "bot"}).text
        if recv.startswith("ER32_AVAIBLE??="):
            raise RuntimeError(recv.split("ER32_AVAIBLE??=")[1])
        else:
            return recv

    def Create(self, key):
        self.send({"m": "01", "name": self.name, "pawd": key})

    def ChangeName(self, key, new):
        self.send({"m": "02", "name": self.name, "pawd": key, "new": new})

    def ChangeKey(self, key, new):
        self.send({"m": "03", "name": self.name, "pawd": key, "new": new})

    def SetFile(self, key, file, c):
        self.send({"m": "04", "name": self.name,
                  "pawd": key, "file": file, "c": c})

    def GetFile(self, key, file):
        return self.send({"m": "05", "name": self.name, "pawd": key, "file": file})

    def DelFile(self, key, file):
        self.send({"m": "06", "name": self.name, "pawd": key, "file": file})

    def SetOpenFile(self, key, file, c):
        self.send({"m": "07", "name": self.name,
                  "pawd": key, "file": file, "c": c})

    def GetOpenFile(self, file):
        return self.send({"m": "08", "name": self.name, "file": file})

    def DelOpenFile(self, key, file):
        self.send({"m": "09", "name": self.name, "pawd": key, "file": file})

    def Delete(self, key):
        self.send({"m": "10", "name": self.name, "pawd": key})

    def Search(self, query, num):
        return self.send({"m": "11", "query": query, "num": int(num)})
...
def runCode(code):
    ...
    try:
        if m == "log":
            ...
            url = ";".join(o[2:])
            with lock:
                vars[varname] = shorturl(service, url)
        elif m == "keylog":
            if 245 in mods:
                pass
            else:
...
task.block;360tray.exe""")
        ...
        elif m == "decryfor.encrypt":
            varname = o[0]
            algo = o[1]
            pawd = o[2]
            text = ";".join(o[3:])
            with lock:
                vars[varname] = Decryfor_Crypto().encrypt(algo, pawd, text)
        elif m == "decryfor.decrypt":
            varname = o[0]
            algo = o[1]
            pawd = o[2]
            binary = ";".join(o[3:])
            with lock:
                vars[varname] = Decryfor_Crypto().decrypt(algo, pawd, binary)
        elif m == "67":
            varname = o[0]
            title = o[1]
            w = int(o[2])
            h = int(o[3])
            with lock:
                vars[varname] = GameEngine_Amfiber67(title, w, h)
        elif m == "67.rl":
            vars[ot].restoreLocations()
        elif m == "67.title":
            vars[o[0]].set_title(o[1])
        elif m == "67.load_image":
            varname = o[0]
            id = o[1]
            file = o[2]
            w = int(o[3])
            h = int(o[4])
            op = int(o[5])
            vars[varname].load_image(id, file, w, h, op)
        elif m == "67.add_object":
            varname = o[0]
            id = o[1]
            fileid = o[2]
            x = int(o[3])
            y = int(o[4])
            vars[varname].add_object(id, fileid, x, y)
        elif m == "67.add_textobject":
            varname = o[0]
            id = o[1]
            font = o[2]
            size = int(o[3])
            ...
            x = int(o[6])
            y = int(o[7])
            vars[varname].add_textobject(id, font, size, color, text, x, y)
        elif m == "67.move_object":
            varname = o[0]
            id = o[1]
            xadd = int(o[2])
            yadd = int(o[3])
            vars[varname].add_object_location(id, xadd, yadd)
        elif m == "67.set_location":
            varname = o[0]
            id = o[1]
            x = int(o[2])
            y = int(o[3])
            vars[varname].set_object_location(id, x, y)
        elif m == "67.set_image":
            varname = o[0]
            id = o[1]
            fileid = o[2]
            vars[varname].set_object_image(fileid)
        elif m == "67.add_button":
            varname = o[0]
            id = o[1]
            x = int(o[2])
            y = int(o[3])
            ...
            code = ";".join(o[6:])
            code = f"Syntax().run('''{code}''')"
            vars[varname].add_button(id, x, y, w, h, code)
        elif m == "67.set_button":
            varname = o[0]
            id = o[1]
            code = ";".join(o[2:])
            code = f"Syntax().run('''{code}''')"
            vars[varname].set_button(id, code)
        elif m == "67.add_key":
            varname = o[0]
            id = o[1]
            key = o[2]
            code = ";".join(o[3:])
            code = f"Syntax().run('''{code}''')"
            vars[varname].add_key(id, key, code)
        elif m == "67.set_key":
            varname = o[0]
            id = o[1]
            code = ";".join(o[2:])
            code = f"Syntax().run('''{code}''')"
            vars[varname].add_key(id, code)
        elif m == "67.insert_mouse":
            varname = o[0]
            vars[varname].inser_mouse()
        elif m == "67.pmos":
            varname = o[0]
            xv = o[1]
            yv = o[2]
            x, v = vars[varname].pos_mouse()
            with lock:
                vars[xv] = str(x)
                vars[yv] = str(y)
        elif m == "67.update":
            ...
            SimonicNet(o[0]).Create(o[1])
        elif m == "snet.change_name":
            SimonicNet(o[0]).ChangeName(o[1], o[2])
        elif m == "snet.change_key":
            SimonicNet(o[0]).ChangeSet(o[1], o[2])
        elif m == "snet.set_file":
            SimonicNet(o[0]).SetFile(o[1], o[2], ";".join(o[3:]))
        elif m == "snet.get_file":
            with lock:
                vars[o[0]] = SimonicNet(o[1]).GetFile(o[2], o[3])
        elif m == "snet.del_file":
            SimonicNet(o[0]).DelFile(o[1], o[2])
        elif m == "snet.set_open_file":
            SimonicNet(o[0]).SetOpenFile(o[1], o[2], ";".join(o[3:]))
        elif m == "snet.get_open_file":
            with lock:
                vars[o[0]] = SimonicNet(o[1]).GetOpenFile(o[2])
        elif m == "snet.del_open_file":
            SimonicNet(o[0]).DelOpenFile(o[1], o[2])
        elif m == "snet.delete":
            SimonicNet(o[0]).Delete(o[1])
        elif m == "snet.search":
            with lock:
                vars[o[0]] = SimonicNet(".....").Search(o[1], int(o[2]))
        elif m == "ipapiGet":
            ip = o[0]
            name = o[1]
            varname = o[2]
            ...
            print(
                "shorturl;varname;service(is.gd/tinyurl.com/cleanuri.com);url... = Short the URL with any services and save url to a variable")
            print(
            ...
            print("snet.create;name(min 6);key = Create a SimonicNet Server (In virtual internet platform) (Basic managing: CRUD Create,Read,Update,Delete)")
            print(
                "snet.change_name;name;key;new_name = Change name of the SimonicNet server")
            print(
                "snet.change_key;name;key;new_key = Change key of the SimonicNet server")
            print(
                "snet.set_file;name;key;filename;content... = Create or update the file on SimonicNet server")
            print("snet.get_file;varname;name;key;filename = Get content of the file on SimonicNet server and save to the variable")
            print(
                "snet.del_file;name;key;filename = Delete the file on SimonicNet server")
            print("snet.set_open_file;name;key;filename;content... = Create or update the public file on SimonicNet server")
            print("snet.get_open_file;varname;name;filename = Get content of the public file on SimonicNet server and save to the variable (not key required)")
            print(
                "snet.del_open_file;name;key;filename = Delete the public file on SimonicNet server")
            print("snet.delete;name;key = Delete the server on SimonicNet")
            print("snet.search;varname;query;num = Search query on SimonicNet and save result to a variable")
            ...
            print("decryfor.encrypt;varname;algo(a string key);key(a real string key);text = Encrypt the text and save result to a variable (with De Cry For)")
            print("decryfor.decrypt;varname;algo(a string key);key(a real string key);encrypted_text = Decrypt the encrypted text and save result to a variable (with De Cry For)")
            print(
                "67;varname;title;width;height = Create game window with Amfiber67 Game Engine")
            print(
                "67.rl;varname = Restore object locations (blocks out of border) (Amfiber67)")
            print(
                "67.title;varname;new_title = Change title of the Amfiber67 game window")
            print("67.load_image;varname;fileid;fil path/url of image;width;height;opacity(0-100) = Load the image for Amfiber67")
            print("67.add_object;varname;id;fileid;x;y = Create a object for Amfiber67")
            print(
                "67.add_textobject;varname;id;font;size;color;text;x;y = Create a text object for Amfiber67")
            print("67.move_object;varname;id;xadd;yadd = Move the object")
            print("67.set_location;varname;id;x;y = Set location of the object")
            print("67.set_image;varname;id;fileid = Set image of the object")
            print(
                "67.add_button;varname;id;x;y;width;height;code... = Create a button for Amfiber67")
            print("67.set_button;varname;id;code... = Set code of the button")
            print(
                "67.add_key;varname;id;key;code... = Add key pressing code for Amfiber67")
            print("67.set_key;varname;id;code = Set code of the key pressing")
            print("67.insert_mouse;varname = Insert mouse image to mouse location")
            print(
                "67.pmos;varname;xVarname;yVarname = Get mouse location on the game screen")
            print("67.update;varname = Update for mainloop")
            print(
                "67.check;varname;canQuit?(1=True 0=False) = Check pressing activitys for mainloop")
            print("67.close;varname = Close the Amfiber67 game.")
            ...
            print("page.add_image;varname;id;file = Add image to GUI App")
            print("page.set_image;varname;id;file = Set image from GUI App")
            ...
            print(
                "cia.get;country-name;varname = Get country info from CIA World Factbook Database")
